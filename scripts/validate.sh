...
grep -q '^VOICE_REPLY_MAX_TOKENS=48$' "$ROOT/shared/config.env.default"
grep -q '^VOICE_REPLY_TEMPERATURE=0.2$' "$ROOT/shared/config.env.default"
grep -q '^VOICE_SYSTEM_PROMPT=' "$ROOT/shared/config.env.default"
grep -q '^QPKG_VER="0.3.4"$' "$ROOT/qpkg.cfg"
...
grep -q 'VOICE_REPLY_MAX_TOKENS' "$ROOT/admin/config.go"
grep -q 'chat_template_kwargs' "$ROOT/admin/voice.go"
grep -q 'reasoning_effort' "$ROOT/admin/voice.go"
grep -q 'llm_predicted_n' "$ROOT/admin/voice.go"
...
grep -q 'loaded lazily only if the' "$ROOT/voiceworker/engine.go"
