...
grep -q '^QPKG_VER="0.4.1"$' "$ROOT/qpkg.cfg"
grep -q '/v1/audio/speech/stream' "$ROOT/admin/main.go"
grep -q 'handleVoiceChatStreamContext' "$ROOT/admin/main.go"
grep -q 'handleVoiceChatContextAdaptive' "$ROOT/admin/main.go"
grep -q 'X-Qnap-Voice-Context' "$ROOT/admin/main.go"
grep -q 'parseVoiceChatInput' "$ROOT/admin/voice_context.go"
grep -q 'voiceEffectiveMaxTokens' "$ROOT/admin/voice_context.go"
grep -q 'voiceLLMMessages' "$ROOT/admin/voice_context.go"
grep -q 'streamVoiceLLMWithControls' "$ROOT/admin/voice_chat_context.go"
grep -q 'llm_history_messages' "$ROOT/admin/voice_chat_context.go"
