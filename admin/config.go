...
func saveConfig(path string, c config) error {
	order := []string{
		...
		"OPENROUTER_BASE_URL", "OPENROUTER_MODEL", "OPENROUTER_FALLBACK_MODELS", "OPENROUTER_AUTO_FREE_FALLBACK", "OPENROUTER_AUTO_FREE_FALLBACK_ATTEMPTS", "OPENROUTER_PRESET", "OPENROUTER_TEMPERATURE", "OPENROUTER_TOP_P", "OPENROUTER_REASONING_EFFORT",
...
func defaults(c config) config {
	...
	defs := config{
		...
		"OPENROUTER_BASE_URL": defaultOpenRouterBaseURL, "OPENROUTER_MODEL": "openrouter/free", "OPENROUTER_FALLBACK_MODELS": "",
		// Automatic cross-model fallback is intentionally opt-in because the model
		// that answers can change. Three total attempts balances availability with
		// the free-tier request quota; users may raise it to at most five.
		"OPENROUTER_AUTO_FREE_FALLBACK": "0", "OPENROUTER_AUTO_FREE_FALLBACK_ATTEMPTS": "3", "OPENROUTER_PRESET": "",
